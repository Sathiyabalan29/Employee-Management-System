...
const EmployeeCard = ({employee, onDelete, onEdit}) => {
    ...
    const handleDelete = () => {
        if (!confirm(`Are you sure you want to delete ${employee.firstName} ${employee.lastName} employee?`)) {
            return
        }
    ...
    return (
    <div className='group relative card card-hover overflow-hidden hover:bg-indigo-50 hover:border-indigo-200 transition-colors'>
        <div className='relative aspect-4/3 w-full overflow-hidden bg-linear-to-br from-slate-100 to-slate-50'>
            
            <div className='w-full h-full flex items-center justify-center '>
                {/* circle icons */}
                <div className='w-20 h-20 rounded-full bg-linear-to-br form -indigo-100 to-slate-100 flex items-center juntify-center'>
        ...
        {!employee.isDeleted && (
            <div className='absolute inset-0 bg-linear-to-t from-indigo-700/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-6 gap-3'>
                ...
                <button onClick={handleDelete} className='p-2.5 bg-white/90 backdrop-blur-sm text-slate-700 hover:text-indigo-600 rounded-xl shadow-lg transition-all hover:scale-105 disabled:opacity-50'>
        ...
        <div className='p-5'>
            <h3 className='text-slate-900 group-hover:text-indigo-700 transition-colors'>{employee.firstName} {employee.lastName}</h3>
            <p className='text-xs text-slate-500 group-hover:text-indigo-600 transition-colors'>{employee.position}</p>
        </div>
    </div>
  ) 
}
